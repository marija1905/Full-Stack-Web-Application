...
class UserController {
    constructor() {
        this.login = (req, res) => {
            ...
            let t = req.body.tip;
            user_1.default.findOne({ korisnicko_ime: u, lozinka: p, tip: t, status: "aktivan" }).then((user) => {
                res.json(user);
            }).catch((err) => {
                console.log(err);
                res.json(null);
        ...
        this.register = async (req, res) => {
            ...
            try {
                ...
                const user1 = await user_1.default.findOne({ korisnicko_ime: korisnik.korisnicko_ime });
                if (user1 != null) {
                    return res.json({ message: "Korisnik sa ovim korisnickim imenom vec postoji." });
                }
                const user2 = await user_1.default.findOne({ email: korisnik.email });
                if (user2 != null) {
                    return res.json({ message: "Korisnik sa ovim email-om vec postoji." });
                }
                new user_1.default(korisnik).save().then(ok => {
        ...
        this.changePassword = (req, res) => {
            ...
            //console.log(u)
            user_1.default.updateOne({ korisnicko_ime: u, lozinka: sl }, { $set: { lozinka: p } }).then(data => {
        ...
        this.getRegTurista = (req, res) => {
            user_1.default.find({ tip: "turista" }).then((user) => {
                res.json(user);
            }).catch((err) => {
                console.log(err);
                res.json(null);
        ...
        this.getRegVlasnik = (req, res) => {
            user_1.default.find({ tip: "vlasnik_vikendice" }).then((user) => {
                res.json(user);
            }).catch((err) => {
                console.log(err);
                res.json(null);
        ...
        this.azuriraj = (req, res) => {
            ...
            //console.log(data)
            //console.log(u)
            user_1.default.findOneAndUpdate({ korisnicko_ime: u }, { $set: data }).then((user) => {
        ...
        this.getSveKorisnike = (req, res) => {
            user_1.default.find({ tip: { $ne: 'admin' } }).then((user) => {
                res.json(user);
            }).catch((err) => {
                console.log(err);
                res.json(null);
        ...
        this.deaktivirajKorisnika = (req, res) => {
            let k = req.body.k;
            user_1.default.findOneAndUpdate({ korisnicko_ime: k.korisnicko_ime }, { $set: { status: "deaktiviran" } }).then((user) => {
                res.json({ message: "Uspesno promenjen status korisniku: " + (user === null || user === void 0 ? void 0 : user.korisnicko_ime) });
            }).catch((err) => {
                console.log(err);
        ...
        this.aktivirajKorisnika = (req, res) => {
            let k = req.body.k;
            user_1.default.findOneAndUpdate({ korisnicko_ime: k.korisnicko_ime }, { $set: { status: "aktivan" } }).then((user) => {
                res.json({ message: "Uspesno promenjen status korisniku: " + (user === null || user === void 0 ? void 0 : user.korisnicko_ime) });
            }).catch((err) => {
                console.log(err);
                res.json({ message: "Greška pri pokusaju promene statusa korisniku." });
        ...
        this.deleteKorisnika = (req, res) => {
            let k = req.body.k;
            user_1.default.deleteOne({ korisnicko_ime: k.korisnicko_ime }).then((user) => {
        ...
        this.odbij = (req, res) => {
            let k = req.body.k;
            user_1.default.findOneAndUpdate({ korisnicko_ime: k.korisnicko_ime }, { $set: { status: "odbijen" } }).then((user) => {
                res.json({ message: "Uspesno promenjen status korisniku: " + (user === null || user === void 0 ? void 0 : user.korisnicko_ime) });
            }).catch((err) => {
                console.log(err);
                res.json({ message: "Greška pri pokusaju promene statusa korisniku." });
